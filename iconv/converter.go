...
// NewConverter Initialize a new Converter. If fromEncoding or toEncoding are not supported by
...
func (c *Converter) Convert(input []byte, output []byte) (bytesRead int, bytesWritten int, err error) {
	...
	if c.open {
		...
		if inputLeft > 0 && outputLeft > 0 {
			...
			_, err = C.call_iconv(c.context, inputPointer, &inputLeft, outputPointer, &outputLeft)
			...
			_, err = C.call_iconv(c.context, nil, &inputLeft, outputPointer, &outputLeft)

			// update write byte counter
			bytesWritten = len(output) - int(outputLeft)
		} else {
			// both input and output are zero length, do a shift state reset
			_, err = C.call_iconv(c.context, nil, &inputLeft, nil, &outputLeft)
		}
	} else {
		err = syscall.EBADF
	}
...
func (c *Converter) ConvertString(input string) (output string, err error) {
	...
	if c.open {
		...
		for totalBytesRead < len(inputBuffer) && err == nil {
			...
			bytesRead, bytesWritten, err = c.Convert(inputBuffer[totalBytesRead:], outputBuffer[totalBytesWritten:])
		...
		if err == nil {
			...
			_, bytesWritten, err = c.Convert([]byte{}, outputBuffer[totalBytesWritten:])
