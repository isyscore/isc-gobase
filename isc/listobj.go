...
//If any two elements would have the same key returned by keySelector the last one gets added to the map.
//The returned map preserves the entry iteration order of the original collection.
func AssociateByAndValue[T, K, V any](list []T, keySelector func(T) K, valueTransform func(T) V) map[K]V {
	r := make(map[K]V)
	for _, e := range list {
...
//If any two elements would have the same key returned by keySelector the last one gets added to the map
func AssociateByTo[T, K any](list []T, keySelector func(T) K, destination map[K]T) map[K]T {
	for _, e := range list {
...
//If any two elements would have the same key returned by keySelector the last one gets added to the map
func AssociateByAndValueTo[T, K, V any](list []T, keySelector func(T) K, valueTransform func(T) V, destination map[K]V) map[K]V {
	for _, e := range list {
...
//The returned map preserves the entry iteration order of the original collection.
func AssociateWith[T, V any](list []T, valueSelector func(T) V) map[T]V {
...
func AssociateWithTo[T, V any](list []T, destination map[T]V, valueSelector func(T) V) map[T]V {
	for _, e := range list {
		destination[e] = valueSelector(e)
	}
	return destination
